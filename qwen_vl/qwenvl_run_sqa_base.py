...
def parse_args():
    parser = argparse.ArgumentParser(description="Qwen-VL base SFT training on ScienceQA")
...
def format_target(example):
    ...
    lecture = (example.get("lecture", "") or "").replace("\n", " ").strip()
    solution = (example.get("solution", "") or "").replace("\n", " ").strip()
    rationale_parts = [part for part in [lecture, solution] if part]
